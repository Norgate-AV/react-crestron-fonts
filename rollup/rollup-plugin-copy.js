...
async function generateCopyTarget(src, dest, { flatten, rename, transform }) {
    if (transform && !(await isFile(src))) {
        throw new Error(`"transform" option works only on files: '${src}' must be a
file`);
    }
        ...
        [hook]: async () => {
            ...
            if (Array.isArray(targets) && targets.length) {
                for (const target of targets) {
                    if (!isObject(target)) {
                        throw new Error(
                            `${stringify(target)} target must be an object`,
                        );
                    }
...
st" properties`);
                    ...
                    if (
                        ...
                        throw new Error(`${stringify(
                            target,
                        )} target's "rename" property mus
t be a string or a function`);
                    }
                ...
                for (const copyTarget of copyTargets) {
                    ...
                    if (transformed) {
                        ...
                        try {
                            ...
                            if (
                                e.code !== "EEXIST" ||
                                restPluginOptions.errorOnExist
                            ) {
                                throw e;
                            }
                        }
                    ...
                    if (verbose) {
                        ...
                        const flags = Object.entries(copyTarget)
                            .filter(
                                ([key, value]) =>
                                    ["renamed", "transformed"].includes(key) &&
                                    value,
                            )
